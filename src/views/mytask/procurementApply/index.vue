  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="申请类型" prop="applicationType">
        <el-select v-model="queryParams.applicationType" placeholder="请选择申请类型" clearable>
          <el-option
            v-for="(dict,index) in applicationTypeArr"
            :key="index"
            :label="dict"
            :value="index"
          />
    ...
    <el-table v-loading="loading" :data="procurementPlanList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="申请类型" align="center" prop="applicationType" >
        <template slot-scope="scope">
          {{applicationTypeArr[scope.row.applicationType]}}
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="45%" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="申请单号" prop="applicationNo">
          <el-input v-model="form.applicationNo" placeholder="请输入申请单号" disabled/>
        </el-form-item>
        <el-form-item label="计划单号" prop="planNo">
        ...
        <el-table :data="form.scmPurchaseApplyItemList" :row-class-name="rowScmSaleOrderItemIndex"
...
<script>
import {getApply, updateApply, listApply, approveApply} from "@/api/scm/apply";
