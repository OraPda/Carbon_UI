  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="1050px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" :inline="true">
        ...
        <el-form-item label="发货单号" prop="deliverNo">
          <el-input v-model="form.deliverNo" disabled/>
        </el-form-item>
        <el-form-item label="客户" prop="customId">
          <el-select v-model="form.customId" disabled>
            <el-option
              v-for="dict in customList"
              :key="dict.id"
              :label="dict.customName"
        ...
        <el-form-item label="收货地址" prop="address">
          <el-input v-model="form.address" placeholder="请输入收货地址"  :disabled="form.applicationStatus!='1'&&form.approvedStatus!='1'?false:true"/>
        </el-form-item>
        <el-form-item label="联系人" prop="contacts">
          <el-input v-model="form.contacts" placeholder="请输入联系人"  :disabled="form.applicationStatus!='1'&&form.approvedStatus!='1'?false:true"/>
        </el-form-item>
        <el-form-item label="联系电话" prop="telephone">
          <el-input v-model="form.telephone" placeholder="请输入联系电话"  :disabled="form.applicationStatus!='1'&&form.approvedStatus!='1'?false:true"/>
        </el-form-item>
        <el-form-item label="承运人" prop="carrier">
          <el-select v-model="form.carrier" placeholder="请选择承运人" clearable :disabled="form.applicationStatus!='1'&&form.approvedStatus!='1'?false:true">
            <el-option
              v-for="dict in userList"
              :key="dict.userId"
              :label="dict.userName"
        ...
        <el-form-item label="运输距离" prop="distance">
          <el-input v-model.number="form.distance" placeholder="请输入运输距离" :disabled="form.applicationStatus!='1'&&form.approvedStatus!='1'?false:true">
        ...
        <el-form-item label="申请状态" prop="applicationStatus">
          <el-select v-model="form.applicationStatus" disabled>
            <el-option
              v-for="dict in dict.type.wms_carrier_application_status"
              :key="dict.value"
              :label="dict.label"
        ...
        <el-form-item label="审核人" prop="approved">
          <el-select v-model="form.approved" disabled>
            <el-option
              v-for="dict in userList"
              :key="dict.userId"
              :label="dict.userName"
        ...
        <el-form-item label="备注" prop="remark">
          <el-input v-model="form.remark" placeholder="请输入备注"  :disabled="form.applicationStatus!='1'&&form.approvedStatus!='1'?false:true"/>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="saveForm()" v-if="form.applicationStatus!='1'&&form.approvedStatus!='1'">保 存</el-button>
        <el-button type="primary" @click="submitForm()" v-if="form.applicationStatus!='1'&&form.approvedStatus!='1'">提 交</el-button>
...
import {
  ...
  delCarrierApplication,
  addCarrierApplication,
  updateCarrierApplication,
  listCustom,
  listUser,
  listWarehouse,
  subCarrierApplication
    ...
    return {
      ...
      rules: {
        ...
        distance: [
          ...
          { type: 'number', message: "运输距离只能为数字值" }
        ],
      },
      // 用户列表
      userList: [],
  ...
  methods: {
    ...
    saveForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          updateCarrierApplication(this.form).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.form.wmsCarrierApplicationDetailsList = this.wmsCarrierApplicationDetailsList;
          subCarrierApplication(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除承运申请编号为"' + ids + '"的数据项？').then(function() {
        return delCarrierApplication(ids);
      }).then(() => {
        this.getList();
